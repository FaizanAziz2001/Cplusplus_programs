...
int CountTrailingZeros(int n)
{
	...
	for (int i = 0; i < Accessbits; i++) {
		if (bit[i] == 0)											//count the number of zeroes
			zero++;
		else
			break;
	}
...
bool find_in_table(int col, int val, vector<vector<Page>>& Table)
{
	for (int i = 0; i < Table.size(); i++)
	{
		if (Table[i][col].id == val - 48)
			return true;											//check if a specific page exists
	}
	return false;
}
...
int find_in_table_index(int col, int val, vector<vector<Page>>& Table)
{
	for (int i = 0; i < Table.size(); i++)
	{
		if (Table[i][col].id == val - 48)									//return index of specific page
			return i;
	}
	return -1;
}
...
void update_values(int col, vector<vector<Page>>& Table)
{
	for (int i = 0; i < Table.size(); i++)
	{
		if (Table[i][col].p == 1)
		{
			Table[i][col].a = Table[i][col].a << 1;								//shift left to increase number of zeroes
		}
	}
}
...
void copy_values(int col, vector<vector<Page>>& Table)
{
	...
	for (int i = 0; i < Table.size(); i++)
	{
		Table[i][col] = Table[i][col - 1];									//copy values of previous entries
	}
}
...
int find_recent(int col, vector<vector<Page>>& Table)
{
	...
	for (int i = 0; i < Table.size(); i++)
	{
		...
		if (zeroes > max_zeroes)										//find the least recently used page by counting the number of zeroes in access byte
		{
			max_zeroes = zeroes;
			max_index = i;
		}
...
bool empty(int col, vector<vector<Page>>& Table)
{
	for (int i = 0; i < Table.size(); i++)
	{
		if (Table[i][col].id == -1)										//check if empty spaces
			return true;
	}
	return false;
}
