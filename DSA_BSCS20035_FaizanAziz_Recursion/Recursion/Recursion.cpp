...
void subarray_iteration(vector<int> data, int si, int ei)
{
	if (ei == data.size())
		return;
	else if (si > ei)
		subarray_iteration(data, 0, ei+1);
	else
	{
		cout << "[ ";
		for (int i = si; i < ei; i++)
		...
		cout << data[ei] << "]" << endl;
		subarray_iteration(data, si+1, ei);
	}

}

void Subarray(vector<int> data)
{
	cout << "[]" << endl;
	subarray_iteration(data,0, 0);
}

void sort(int temp,stack<int>& s)
{
	if (s.empty())
	{
		s.push(temp);
